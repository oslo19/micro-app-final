...
const shapePatterns = {
  easy: [
    ...
    },
    {
      ...
      explanation: 'New shape is added each time'
    }
  ],
  medium: [
    {
      ...
      explanation: 'One square is added each time'
    }
  ],
  hard: [
    {
      sequence: '●○, ●●○, ●●●○, ?',
      answer: '●●●●○',
      hint: 'Look at filled and empty circles',
      explanation: 'Filled circles increase, empty circle stays'
    }
  ]
};
  ...
  easy: [
    {
      ...
      explanation: 'First becomes previous second, second adds more'
    },
    {
      sequence: 'Spring→Summer, Summer→Autumn, Autumn→?',
      answer: 'Winter',
      hint: 'Think about seasons',
      explanation: 'Follows seasonal order'
    }
  ],
  medium: [
    {
      ...
      answer: 'Triangle',
      hint: 'Look at size and shape patterns',
      explanation: 'Size increases while shapes change'
    }
  ]
};
...
const generatePattern = async (req, res) => {
    try {
        ...
        // Use pattern collections for shape and logical types
        if (requestedType === 'shape') {
            const patterns = shapePatterns[requestedDifficulty];
            pattern = patterns[Math.floor(Math.random() * patterns.length)];
        } else if (requestedType === 'logical') {
            const patterns = logicalPatterns[requestedDifficulty];
            pattern = patterns[Math.floor(Math.random() * patterns.length)];
        } else {
            // Existing logic for numeric and symbolic patterns
            // ... rest of your pattern generation code
        }
